...
export function useGetOrders() {
    return useMutation('ordersList', () =>
        Fetch.get<orderFromAPI[]>('order').then(orders =>
            orders.map(
                ({ _id, createdAt, tableId, tableName, customer, items }) => ({
                    id: _id,
                    createdAt,
                    tableId,
                    tableName,
...
export function useOrderDone(id: string) {
    return useMutation(['patchOrder', id], () => Fetch.patch(`order/${id}`));
}
