...
import { tableAPI, useGetTables, usePostTable } from '../../Api/table.api';
...
export function TablesRegister() {
    ...
    const {
        ...
        new Array(Number(numberOfTables)).fill('').map((_, index) => ({
    ...
    return (
        <FlexContainer>
            <FlexContent>
                ...
                />
                <Title>{editing ? 'Editar mesas' : 'Cadastrar mesas'}</Title>
                    ...
                    <ListContainer>
                        {currentTables.map((table, index) => (
                            <TableItem
                                table={table}
                                key={table._id}
                                isLastItem={index === currentTables.length - 1}
                ...
                <Form onSubmit={submit}>
                    ...
                    />
                    <Button
                        disabled={!validForm || isLoadingPost || isLoadingGet}
                    >
                        Adicionar
                    </Button>
                </Form>
