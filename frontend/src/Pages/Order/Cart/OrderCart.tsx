...
import { ValueContainer, ItemsContainer } from './OrderCart.styles';
...
export function OrderCart() {
    ...
    const selectedItems = useAppSelector(({ orderRegister }) => orderRegister);
    const allItems = useAppSelector(({ menuInfo }) =>
        menuInfo.categories
            .filter(
                ({ name }) => !['Mais caros', 'Mais baratos'].includes(name)
            )
            .flatMap(({ name, items }) =>
    ...
    const items = selectedItems.map(selectedItem => ({
        ...(allItems.find(item => item.id === selectedItem.id) as item),
        ...selectedItem
    }));
